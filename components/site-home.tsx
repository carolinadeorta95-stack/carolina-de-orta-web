...
export function SiteHome({ properties }: { properties: Property[] }) {
  ...
  return (
    <main className="site-shell">
      ...
      <section id="propiedades" className="properties-section section-pad gray-section"><div className="section-heading"><div><div className="section-kicker">02 / PROPIEDADES</div><h2>Espacios para<br /><em>habitar.</em></h2></div><a className="text-link" href="#contacto">Ver todas <ArrowRight size={16} /></a></div><div className="property-grid">{displayProperties.map((property, index) => { const fallback = fallbackProperties[index % fallbackProperties.length]; const image = 'cover_image' in property ? property.cover_image || fallback.image : property.image; const type = 'status' in property ? property.status || fallback.type : property.type; const price = typeof property.price === 'number' ? `USD ${property.price.toLocaleString('es-AR')}` : property.price; const area = typeof property.area === 'number' ? `${property.area} m²` : property.area; return <article className={index === 0 ? 'property-card featured' : 'property-card'} key={property.id || property.title}><div className="property-image"><Image src={image} alt={property.title} fill sizes="(max-width: 700px) 100vw, 33vw" /><span className="image-number">0{index + 1}</span></div><div className="property-meta"><p className="eyebrow">{type}</p><h3>{property.title}</h3><p>{property.location}</p><div className="property-bottom"><span>{price}</span><span>{area}</span></div></div></article> })}</div></section>
